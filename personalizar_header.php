...
// Interfaz preferida (opcional). Déjalo vacío para AUTO.
$PREFERRED_IF = ''; // ej: 'enp4s0' si quieres forzar

// ⚙️ AUTO: se define más abajo con pick_eth_iface()
$ETH_IF  = '';       // Interfaz Ethernet principal (AUTO)
...
/* =========================================================
   ✅ REPARACIÓN: AUTO-DETECCIÓN DE INTERFAZ ETHERNET REAL
   - Usa la interfaz del default route (ip route default)
   - Valida que sea Ethernet según NetworkManager
   - Si falla, toma la primera Ethernet conectada
   ========================================================= */

function detect_default_iface(): ?string {
    $dev = trim(@shell_exec("ip route show default 2>/dev/null | awk '{print \$5}' | head -n 1"));
    return $dev !== '' ? $dev : null;
}

function nm_device_type(string $dev): string {
    $dev_safe = addslashes($dev);
    $type = trim(@shell_exec("nmcli -t -f DEVICE,TYPE device status 2>/dev/null | awk -F: '\$1==\"{$dev_safe}\" {print \$2}'"));
    return strtolower($type);
}

function nm_device_state(string $dev): string {
    $dev_safe = addslashes($dev);
    $state = trim(@shell_exec("nmcli -t -f DEVICE,STATE device status 2>/dev/null | awk -F: '\$1==\"{$dev_safe}\" {print \$2}'"));
    return strtolower($state);
}

function nm_is_ethernet(string $dev): bool {
    return nm_device_type($dev) === 'ethernet';
}

function nm_find_connected_ethernet(): ?string {
    $out = trim(@shell_exec("nmcli -t -f DEVICE,TYPE,STATE device status 2>/dev/null"));
    if ($out === '') return null;
    ...
    foreach (explode("\n", $out) as $line) {
        ...
        [$dev, $type, $state] = $p;
        if (strtolower($type) === 'ethernet' && strtolower($state) === 'connected') {
            return $dev;
        }
    }
    return null;
...
function pick_eth_iface(string $preferred = ''): string {
    $preferred = trim($preferred);

    // 1) Si el usuario forzó una interfaz y existe como ethernet, úsala
    if ($preferred !== '' && nm_is_ethernet($preferred)) return $preferred;

    // 2) Interfaz que saca internet (default route) si es ethernet
    $def = detect_default_iface();
    if ($def && nm_is_ethernet($def)) return $def;

    // 3) Primera ethernet conectada
    $eth = nm_find_connected_ethernet();
    if ($eth) return $eth;
...
/* ===========================
   I18N
   =========================== */
$langFile = __DIR__ . '/data/lang/' . ($config['idioma'] ?? 'es') . '.json';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf']) || $_POST['csrf'] !== $_SESSION['csrf']) {
        ...
        if ($action === 'save_header') {
            ...
            $idioma = $_POST['idioma'] ?? ($config['idioma'] ?? 'es');
        ...
        /* ---------- CONFIGURAR IP ETHERNET (nmcli) ---------- */
            ...
            if ($ip === '' && $gw === '' && $dns === '') {
                ...
                // ✅ asegurar ETH_IF AUTO incluso en POST
                $ETH_IF = pick_eth_iface($GLOBALS['PREFERRED_IF'] ?? '');
...
// ✅ Interfaz Ethernet real (AUTO)
$ETH_IF = pick_eth_iface($PREFERRED_IF);
...
?>
<!DOCTYPE html>
<html lang="<?= htmlspecialchars($config['idioma'] ?? 'es') ?>">
<head>
<meta charset="UTF-8">
<title><?= __('config_title') ?></title>
      ...
      <div class="col-md-4 text-center mt-1 mt-md-0 d-flex flex-column align-items-center">
        ...
        <div class="d-flex flex-wrap justify-content-center header-tools">
          <a href="index.php" class="btn btn-ghost btn-xxs btn-icon" data-bs-toggle="tooltip"
             title="<?= __('go_dashboard') ?>" aria-label="Dashboard">
...
<div class="container pb-4">
  ...
  <div class="card bg-dark border-secondary mb-4">
    <div class="card-body">
      <h5 class="mb-3"><i class="fas fa-pen-to-square me-2"></i><?= __('customize_header') ?></h5>
...
<div class="mb-3">
  <label class="form-label"><?= __('dashboard_language') ?></label>
  <select name="idioma" class="form-select">
    <option value="es" <?php echo (($config['idioma'] ?? 'es') === 'es') ? 'selected' : ''; ?>>
      🇨🇱 Español
    </option>
    <option value="en" <?php echo (($config['idioma'] ?? 'es') === 'en') ? 'selected' : ''; ?>>
      🇺🇸 English
    </option>
  </select>
  <small class="text-muted d-block mt-1">
    Cambia el idioma de todo el dashboard (index, about, estadísticas, etc).
  </small>
</div>
        ...
        <div class="d-flex gap-2">
          <button type="submit" class="btn btn-warning"><i class="fas fa-save me-1"></i><?= __('save_changes') ?></button>
          <a class="btn btn-outline-light" href="index.php"><i class="fas fa-eye me-1"></i><?= __('view_live') ?></a>
        </div>
      </form>
    </div>
  </div>
  ...
  <div class="card bg-dark border-secondary mb-4">
    <div class="card-body">
      <h5 class="mb-3"><i class="fas fa-file-code me-2"></i><?= __('p25_config') ?></h5>
      <?php
        $ini_txt = '';
        if (file_exists($p25_ini_file)) {
  ...
  <div class="card bg-dark border-secondary mb-4">
    <div class="card-body">
      <h5 class="mb-3"><i class="fas fa-server me-2"></i><?= __('p25_service_control') ?></h5>
      <form method="post" class="d-flex flex-wrap gap-2">
        ...
        <button name="cmd" value="reboot"  class="btn btn-secondary btn-sm"
          ...
          <i class="fas fa-power-off me-1"></i><?= __('reboot_server') ?>
        </button>
      </form>
     </div>
  </div>
...
<div class="card bg-dark border-warning mb-4">
  <div class="card-body">
    <h5 class="mb-3 text-warning">
      <i class="fas fa-broom me-2"></i><?= __('radioid_maintenance') ?></h5>
  ...
  <div class="card bg-dark border-secondary mb-4">
    <div class="card-body">
      <h5 class="mb-3"><i class="fab fa-telegram me-2"></i><?= __('telegram_config') ?></h5>
      <form method="post" autocomplete="off">
        <input type="hidden" name="csrf" value="<?php echo htmlspecialchars($_SESSION['csrf']); ?>">
  ...
  <div class="card bg-dark border-secondary mb-4">
    <div class="card-body">
      <h5 class="mb-3"><i class="fas fa-user-shield me-2"></i><?= __('admin_credentials') ?></h5>
      <form method="post" autocomplete="off">
        <input type="hidden" name="csrf" value="<?php echo htmlspecialchars($_SESSION['csrf']); ?>">
